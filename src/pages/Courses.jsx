...
import { FaSearch, FaFilter, FaTimes, FaWhatsapp, FaInfoCircle, FaCheck, FaBookOpen, FaTimesCircle } from 'react-icons/fa';
    ...
    return (
        <div className="fixed inset-0 bg-black/90 z-[5000] flex items-center justify-center p-4 backdrop-blur-sm">
            <motion.div 
                ...
                className="bg-white dark:bg-[#111] w-full max-w-2xl rounded-2xl p-6 md:p-10 relative overflow-y-auto max-h-[90vh] border border-blue-500/30 shadow-2xl"
            >
                ...
                <h2 className="text-2xl font-bold text-gray-800 dark:text-white mb-2">Enrollment Application</h2>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Apply for: <span className="text-blue-600 dark:text-blue-400 font-bold">{course.title}</span></p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="flex flex-col gap-1">
                            ...
                            <input required type="text" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} 
                                className="p-3 rounded-lg bg-gray-50 dark:bg-[#222] border border-gray-200 dark:border-[#333] outline-none focus:border-blue-500 dark:focus:border-blue-500 dark:text-white transition-colors" />
                        </div>
                        <div className="flex flex-col gap-1">
                    ...
                    <div className="flex flex-col gap-1">
                     ...
                     <div className="flex flex-col gap-1">
                        ...
                        <textarea rows={3} value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})} 
                            className="p-3 rounded-lg bg-gray-50 dark:bg-[#222] border border-gray-200 dark:border-[#333] outline-none focus:border-blue-500 dark:focus:border-blue-500 dark:text-white transition-colors resize-none" placeholder="Any specific queries?" />
                    </div>

                    <div className="bg-blue-50 dark:bg-blue-900/10 p-4 rounded-lg flex items-start gap-3 border border-blue-100 dark:border-blue-500/20 mt-2">
                        <FaInfoCircle className="text-blue-500 mt-1 shrink-0" />
                        <p className="text-xs text-blue-700 dark:text-blue-300 leading-relaxed">
                            By submitting this form, your application will be sent to our admin team. You will be contacted via phone or email for further enrollment steps.
                        </p>
                    </div>

                    <button disabled={loading} type="submit" className="mt-4 bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-xl font-bold transition-all shadow-lg shadow-blue-500/20 disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-[4000] flex items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
            ...
            >
                ...
                <span className="text-xs bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-300 px-2 py-1 rounded inline-block mb-4">{course.branch}</span>
                ...
                <div className="mb-6 space-y-4">
                     ...
                     <div className="flex gap-4">
                        ...
                        <div className="flex-1 bg-yellow-50 dark:bg-yellow-900/10 p-3 rounded-lg border border-yellow-100 dark:border-yellow-800/30 text-center">
                            <span className="block text-xs font-bold text-yellow-600 uppercase">Fees</span>
                ...
                <button 
                    ...
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white py-3 rounded-xl font-bold shadow-lg transition-transform hover:scale-[1.02]"
                >
                    Enroll Now
                </button>
            </motion.div>
...
const Courses = () => {
  ...
  const branches = ['All', ...new Set(coursesData.map(c => c.branch))];
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0B1120] text-gray-900 dark:text-white pt-24 pb-12 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center gap-4 mb-8 border-b border-gray-200 dark:border-gray-800 pb-6">
            <div>
                ...
                <p className="text-gray-500 dark:text-gray-400 text-sm">Discover {coursesData.length}+ certification programs across various domains.</p>
            </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 w-full md:w-auto">
                 <div className="relative">
                    ...
                    <input 
                        ...
                        className="pl-10 pr-4 py-2 w-full md:w-64 rounded-full bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-gray-700 outline-none focus:border-blue-500 transition-colors"
                    />
                 </div>
                 <select 
                    value={selectedBranch}
                    onChange={(e) => setSelectedBranch(e.target.value)}
                    className="px-4 py-2 rounded-full bg-white dark:bg-[#1E293B] border border-gray-200 dark:border-gray-700 outline-none cursor-pointer focus:border-blue-500 transition-colors"
                 >
                    {branches.map(b => (
            ...
            <AnimatePresence>
                {filteredCourses.map((course) => (
                    <motion.div 
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        exit={{ opacity: 0, scale: 0.9 }}
                        className="bg-white dark:bg-[#1E293B] rounded-xl overflow-hidden shadow-sm hover:shadow-xl border border-gray-100 dark:border-gray-800 transition-all cursor-pointer group flex flex-col h-full"
                        ...
                        <div className="p-5 flex-1 flex flex-col">
                            <div className="flex justify-between items-start mb-3">
                                <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider ${
                                    ...
                                    course.branch === 'Mechanical' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300' :
                                    course.branch === 'Civil' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300' :
                            ...
                            <h3 className="text-lg font-bold mb-2 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors line-clamp-2">{course.title}</h3>
                            ...
                            <div className="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700 flex justify-between items-center">
                                <div className="flex flex-col">
                                    ...
                                    <span className="font-bold text-blue-600 dark:text-blue-400 text-lg">{course.price}</span>
                                </div>
                                <button className="text-gray-500 dark:text-gray-400 text-xs font-bold hover:text-blue-600 transition-colors">View Details</button>
                            </div>
                        </div>
                    </motion.div>
                ))}
      ...
      {/* Footer specific to Courses page (hidden if using global logic, but here for safety/separation) */}
      <div className="mt-12">
        <CoursesFooter />
      </div>
